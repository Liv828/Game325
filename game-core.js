...
function compareRows(rowA, rowB) {  //两行对比，防止爆牌
    ...
    if (evalA.type === "对子") {
        if (evalA.details.pairValue !== evalB.details.pairValue) return evalA.details.pairValue - evalB.details.pairValue;
        ...
        for (let i = 0; i < maxLen; i++) {
            const valA = i < sortedA.length ? sortedA[i] : 0;
            const valB = i < sortedB.length ? sortedB[i] : 0;
...
function getSpecialBonus(rows) {  //特殊牌型
    ...
    else if (dragon && eachRowColorsDistinct) best = 20; //异龙，龙+每行颜色都不一样
